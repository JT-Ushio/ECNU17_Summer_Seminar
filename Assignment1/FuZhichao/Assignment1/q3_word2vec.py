...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    ### YOUR CODE HERE
    vhat = predicted
    yhat = softmax(np.dot(outputVectors, vhat))
    cost = -np.log(yhat[target])
    # grad = vhat(yhat - y).T
    # gradPred = U.T(yhat - y)
    z = yhat.copy()
    z[target] -= 1.0
    grad = z.reshape((z.shape[0], 1)) * vhat
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    cost = -np.log(z)
    # grad[uo] = (sigmoid(uo.T dot vhat) - 1)vhat
    # grad[uk] = -(sigmoid(-uk.T dot vhat) - 1)vhat
    # gradPred = -(sigmoid(uo.T dot vhat) - 1)uo - sum(sigmoid(-uk.t dot vhat) - 1)uk
